...
if use_tensorboard:
    log_dir = './checkpoints/spgan_last'
    utils.mkdir(log_dir)
...
""" model """
Da = models.Discriminator().to(device)
Db = models.Discriminator().to(device)
Ga = models.Generator().to(device)
Gb = models.Generator().to(device)
Me = models.Metric_Net().to(device)
MSE = nn.MSELoss()
L1 = nn.L1Loss()
    ...
    for i, (a_real, b_real) in enumerate(itertools.izip(a_loader, b_loader)):
        ...
        if i % 100 == 0:
            with torch.no_grad():
                ...
                save_dir = './sample_images_while_training_spgan_last'
